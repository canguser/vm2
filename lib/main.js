...
class NodeVM extends VM {
	...
	run(code, filename) {
		...
		try {
			...
			// update by Cangshi
			const realResult = wrapper === 'commonjs' ? module.exports : returned;
			const proxieResult = this._internal.Decontextify.value(realResult);

			return new Proxy(proxieResult,{
				apply(target, thisArg, argArray) {
					return Reflect.apply(realResult, thisArg, argArray)
				}
			})
		} catch (e) {
			throw this._internal.Decontextify.value(e);
